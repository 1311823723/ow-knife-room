...
- `player_state` 当前主要定义 `0..115`、`175..188`、`190..209`、`314..319`，其中 `38..44` 为半藏新增技能状态索引，`45..49` 为奥丽莎强固裁决状态索引，`50..65` 为小美寒气掌控状态索引，`66..80` 为伊拉锐召唤太阳状态索引，`81..95` 为生命之梭园艺保龄球状态索引，`96..115` 为莱因哈特荣耀骑士审判状态索引，`175..188` 为无漾状态索引，`190..209` 为旧入侵兼容和狂鼠状态索引，`314..319` 为法老之鹰天隼俯冲状态索引。
...
| `player_state[314]` | `pharah_eagle_momentum` | 法老之鹰鹰势层数，0 到 3，死亡保留，换英雄清空 |
| `player_state[315]` | `pharah_dive_active` | 法老之鹰技能 1 俯冲预备和天隼俯冲流程锁 |
| `player_state[316]` | `pharah_dive_pos` | 法老之鹰本次俯冲准星目标位置 |
| `player_state[317]` | `pharah_melee_gain_cd` | 法老之鹰近战获得鹰势短节流 |
| `player_state[318]` | `pharah_empowered_dive` | 法老之鹰本次俯冲是否为满鹰势强化俯冲 |
| `player_state[319]` | `pharah_dive_targets` | 法老之鹰本次俯冲结算命中的临时目标数组 |
